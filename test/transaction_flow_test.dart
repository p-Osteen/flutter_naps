...
class MockNapsConnection implements NapsConnection {
  ...
  final List<NapsMessage> _responseQueue = []; // Responses EPT will output when client sends a message
  final List<NapsMessage> _receivedBuffer = []; // Messages received and buffered waiting to be read by client
  ...
  Future<void> send(NapsMessage message) async {
    ...
    // Simulate EPT responding immediately to a sent message
...
void main() {
  group('NapsSdk Transaction Flow Tests', () {
    ...
    setUp(() {
      ...
      sdk = NapsSdk(connection: connection, posId: posId, initialSequenceNumber: 100);
      connection.connect();
    });

    test('networkTest returns true when EPT responds successfully', () async {
      // Queue EPT's response (TM 109, CR 000)
      connection.queueResponse(NapsMessage.fromElements([
        TlvElement('001', '109'),
    ...
    test('pay with autoConfirm executes the complete payment-to-confirmation flow', () async {
    ...
    test('onApproved exception returns NapsResultSource.sdkError result after TM=002 is still sent', () async {
      // Queue approved payment response
      connection.queueResponse(NapsMessage.fromElements([
        TlvElement('001', '101'),
        TlvElement('003', posId),
      ...
      // The exception from pay() is caught by the outer try/catch
      // and returned as a failure result with source sdkError
      final result = await sdk.pay(
        amountInCents: 200,
        autoConfirm: true,
        onApproved: (intermediate) async {
      ...
      // TM=002 was still sent despite callback failure
      expect(connection.sentMessages.length, 2);
      expect(connection.sentMessages[1].messageType, '002');

      // The exception propagated as an SDK error
      expect(result.isSuccess, isFalse);
    ...
    test('initiate and confirm cancellation 2-step flow works', () async {
      ...
      expect(connection.sentMessages[1].sequenceNumber, 101); // Identical to TM 003
    });
  ...
  group('Response Code Mapping Tests', () {
    test('response code 480 (cancellation already done) is properly mapped', () {
    ...
    test('response codes NOT in integration reference (995, 280) use declined fallback', () {
      // 995 is not in the reference → declined fallback
      ...
      // 280 is not in the reference → declined fallback
  ...
  group('Cancel Token Tests', () {
    ...
    test('cancel token does NOT interfere when EPT responds before cancel', () async {
  ...
  group('Reconnect During TM=002 Tests', () {
    ...
    test('reconnect is attempted when send fails during TM=002 confirmation', () async {
    ...
    test('reconnect failure during TM=002 returns NapsResultSource.sdkError', () async {
      // Queue approved payment response (TM=101)
      connection.queueResponse(NapsMessage.fromElements([
        TlvElement('001', '101'),
        TlvElement('003', posId),
  ...
  group('Cancellation Response Code 480 Tests', () {
    ...
    test('cancellation confirmation with CR=480 ("already done") is treated as success', () async {
      // Initiate
      connection.queueResponse(NapsMessage.fromElements([
        TlvElement('001', '103'),
        TlvElement('003', posId),
  ...
  group('Timeout Tests', () {
    ...
    test('payment timeout returns NapsResultSource.sdkError with timeout description', () async {
      ...
      expect(result.source, NapsResultSource.sdkError);
      expect(result.responseCode, isEmpty);
      expect(result.description.toLowerCase(), contains('timeout'));
